...
# defaultBranchRef`, and the ruleset is named `protect-<branch>` (for this
# repository that is `protect-master`, since its default branch is `master`).
# Hardcoding `main` on a repo whose default branch is `master` would silently
# protect a branch that does not exist.
#
...
#
# Env overrides:
#   RULESET_NAME=protect-master   BRANCH=master   GH_ACTIONS_APP_ID=15368
